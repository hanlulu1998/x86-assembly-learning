...
;-------------------------------------------------------------------------------
    ...
    export_sym_entity_len   equ $-export_sym4
...
.insatll_gate:
    ...
    ;对门进行测试
    mov ebx,message2
    call far [export_sym_1+256]              ;通过门显示信息(偏移量将被忽略)

    mov ebx,message3
    call sys_routine_seg_sel:print_string ;在内核中调用例程不需要通过门

    ;创建任务控制块。这不是处理器的要求，而是我们自己为了方便而设立的
    mov ecx,0x46
    call sys_routine_seg_sel:sys_malloc
    call append_to_tcb_list            ;将任务控制块追加到TCB链表
